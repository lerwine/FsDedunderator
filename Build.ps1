...
$Path = "HKLM:\SOFTWARE\Microsoft\MSBuild\ToolsVersions";
$MSBuildExePath = $null;
$VersionInfo = @();
if (Test-Path -LiteralPath $Path -PathType Container) {
    $VersionInfo = @((Get-ChildItem -LiteralPath $Path) | ForEach-Object {
        $ToolsVersionPath = $Path | Join-Path -ChildPath $_.PSChildName;
        $Version = New-Object -TypeName 'System.Version';
        if ([Version]::TryParse($_.PSChildName, [ref]$Version) -and $_.Property -contains 'MSBuildToolsPath') {
            $PropertyValues = @{
                Version = $Version;
                MSBuild = (Get-ChildItem -LiteralPath ((Get-ItemPropertyValue -LiteralPath $ToolsVersionPath -Name 'MSBuildToolsPath'))) | Where-Object { $_.Name -ieq 'msbuild.exe'};
            };
            if ($null -ne $PropertyValues['MSBuild']) {
                New-Object -TypeName 'System.Management.Automation.PSObject' -Property $PropertyValues;
            }
        }
        
    });
}
$DirectoryInfo = (Get-ChildItem -LiteralPath ([Environment]::GetFolderPath([Environment+SpecialFolder]::ProgramFilesX86)) -Directory) | Where-Object { $_.Name -ieq 'Microsoft Visual Studio'} | Select-Object -First 1;
if ($null -ne $DirectoryInfo) {
    Write-Debug -Message "Searching $($DirectoryInfo.FullName)";
    $VersionInfo += @(Get-ChildItem -LiteralPath $DirectoryInfo.FullName | ForEach-Object {
        $DirectoryInfo = (Get-ChildItem -LiteralPath $_.FullName -Directory) | Where-Object { $_.Name -ieq 'BuildTools'} | Select-Object -First 1;
        if ($null -ne $DirectoryInfo) {
            Write-Debug -Message "Searching $($DirectoryInfo.FullName)";
            $DirectoryInfo = (Get-ChildItem -LiteralPath $DirectoryInfo.FullName -Directory) | Where-Object { $_.Name -ieq 'MSBuild'} | Select-Object -First 1;
            if ($null -ne $DirectoryInfo) {
                Write-Debug -Message "Searching $($DirectoryInfo.FullName)";
                (Get-ChildItem -LiteralPath $DirectoryInfo.FullName -Directory) | ForEach-Object {
                    $n = $_.Name;
                    if ($n.Substring(0, 1) -ieq 'v' -and $n.Length -gt 1) { $n = $N.Substring(1) }
                    Write-Debug -Message "Matching version on $n";
                    $Version = New-Object -TypeName 'System.Version';
                    if ([Version]::TryParse($n, [ref]$Version)) {
                        Write-Debug -Message "Got version $Version"
                        $FileInfo = $null;
                        $DirectoryInfo = (Get-ChildItem -LiteralPath $_.FullName -Directory) | Where-Object { $_.Name -ieq 'bin'} | Select-Object -First 1;
                        if ($null -ne $DirectoryInfo) {
                            Write-Debug -Message "Searching $($DirectoryInfo.FullName)";
                            $FileInfo = (Get-ChildItem -LiteralPath $DirectoryInfo.FullName -File) | Where-Object { $_.Name -ieq 'msbuild.exe'} | Select-Object -First 1;
                        }
                        if ($null -eq $FileInfo) {
                            Write-Debug -Message "Searching $($_.FullName)";
                            $FileInfo = (Get-ChildItem -LiteralPath $_.FullName -File) | Where-Object { $_.Name -ieq 'msbuild.exe'} | Select-Object -First 1;
                        }
                        if ($null -ne $FileInfo) {
                            Write-Debug -Message "Returning $($FileInfo.FullName) $Version";
...
$MSBuildExePath = @($VersionInfo | Sort-Object -Property 'Version') | ForEach-Object { $_.MSBuild.FullName } | Select-Object -Last 1;
if ($null -eq $MSBuildExePath) {
    $MSBuild = (Get-ChildItem -LiteralPath ([System.Runtime.InteropServices.RuntimeEnvironment]::GetRuntimeDirectory())) | Where-Object { $_.Name -ieq 'msbuild.exe' };
    if ($null -ne $MSBuild) { $MSBuildExePath = $MSBuild.FullName }
}
if ($null -eq $MSBuildExePath) {
    Write-Warning -Message "Unable to determine the location of MSBuild.exe";
} else {
    $LogFiles = @{
        minimal = 'msbuild.sum';
        warningsonly = 'msbuild.wrn';
        errorsonly = 'msbuild.err';
    };
    Push-Location -LiteralPath $PSScriptRoot;
    try {
        $LogFiles.Keys | ForEach-Object {
            if ($LogFiles[$_] | Test-Path) {
                Remove-Item -Path $LogFiles[$_] -Force -ErrorAction Stop;
            }
        }
        "MSBuild found at $MSBuildExePath" | Write-host
        . $MSBuildExePath "/t:$($Targets -join ';')" "/p:GenerateFullPaths=true;Configuration=`"$Configuration`";Platform=`"$Platform`"" 'FsDedunderator.sln' `
            "/flp1:minimal;logfile$($LogFiles['minimal'])" "/flp2:warningsonly;logfile=$($LogFiles['warningsonly'])" "/flp3:errorsonly;logfile=$($LogFiles['errorsonly'])";
    }
